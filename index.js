...
<html>
  <head>
    ...
    <title>Todos on Macrometa KV DB</title>
...
// Remove this once KV apis are available --------
...
const setCache = (key, data) => {
  ...
  return client.insertKVPairs(collectionName, raw)
}

const getCache = key => {
  return client.getValueForKey(collectionName, key)
}
...
async function getTodos(request) {
  ...
  try {
    ...
    myKey = `data-${ip}`
    const parsed = await getCache(myKey)
    data = parsed.value
  } catch (e) {
    await setCache(myKey, defaultData)
    data = defaultData
  }
